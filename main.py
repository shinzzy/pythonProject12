...
class GraphWindow(QMainWindow, ui_form2):
    def __init__(self):
        ...
        self.pushButton_goback.setIcon(QIcon("./goback.png"))
    ...
    def total(self):
        ...
        self.ax.clear()
        self.ax2.clear()
        self.ax.set_ylabel("AVG price by company (WON) ")
        ...
        for i in self.mymy:
            space1 = 0
            space2 = 0
            sql = '''Select price, reviewCount From outers where company = "{}"
                     UNION ALL
                     Select price, reviewCount From pants where company = "{}"
                     UNION ALL
                     Select price, reviewCount From shirts where company = "{}"'''.format(i, i, i)
            cur.execute(sql)
            result = cur.fetchall()
            print(result)
            for j in result:
                space = j[0].replace(',', '')
                space = space.replace('원', '')
                space1 += int(space)

                space_ = j[1].replace(',', '')
                space_ = space_.replace('건', '')
                space2 += int(space_)

            if i == "11번가":
                price_11 = int(space1 / 30)
                rev_11 = int(space2 / 30)
                print(price_11)
                print(rev_11)
            elif i == "G마켓":
                price_gm = int(space1 / 30)
                rev_gm = int(space2 / 30)
                print(price_gm)
                print(rev_gm)
            elif i == "옥션":
                price_auc = int(space1 / 30)
                rev_auc = int(space2 / 30)
                print(price_auc)
                print(rev_auc)
            else:
                price_shin = int(space1 / 30)
                rev_shin = int(space2 / 30)
                print(price_shin)
                print(rev_shin)
        topics = ['11st', 'Gmarket', 'Auction', 'Shinsegae']  # x축 이름정하기
        ...
        self.ax.bar(value_a_x, value_a, color='lightgreen', label='avg_price')
        # self.ax.set_ylabel("AVG price by company (WON) ")


        # self.ax2 = self.ax.twinx()
        self.ax2.bar(value_b_x, value_b, color='yellow', label='avg_review')
        self.ax2.set_ylabel("AVG rieview by company")
    ...
    def first(self):
        ...
        self.ax.bar(value_a_x, value_a, color='pink', label='price')
        ...
        self.ax2.bar(value_b_x, value_b, color='skyblue', label='review')
